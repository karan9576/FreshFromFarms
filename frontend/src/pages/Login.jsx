...
export default function Login({ setUser }) {
  ...
  const handleVerify = async (e) => {
    ...
    try {
      const res = await axios.post(`${cleanApiURL}/auth/verify-email`, {
  ...
  const handleResendCode = async () => {
    ...
    setResendStatus('');
    try {
      const res = await axios.post(`${cleanApiURL}/auth/resend-code`, {
        email: verificationEmail
      });
      setResendStatus(res.data.message || 'Verification code resent successfully.');
    } catch (err) {
      setLocalError(err.response?.data?.message || 'Failed to resend code.');
    }
  };
      ...
      <div className="glass-panel" style={{ maxWidth: '450px', width: '100%', padding: '2.5rem 2rem', boxSizing: 'border-box' }}>
        ...
        {showVerificationScreen ? (
          <>
            ...
            <p style={{ color: 'var(--text-muted)', marginBottom: '2rem', fontSize: '0.9rem', textAlign: 'center', lineHeight: '1.5' }}>
              We have sent a 6-digit verification code to <strong>{verificationEmail}</strong>. Please enter it below to confirm your account.
            </p>
            ...
            {localError && (
              <div style={{ 
                ...
                <p style={{ color: '#e74c3c', fontWeight: 700, margin: 0 }}>Verification Error</p>
                <p style={{ color: 'var(--text-dark)', margin: '0.2rem 0 0', opacity: 0.85 }}>{localError}</p>
              </div>
            )}

            {resendStatus && (
              <div style={{ 
                backgroundColor: 'rgba(12, 56, 35, 0.08)', 
                ...
                <p style={{ color: 'var(--primary-color)', fontWeight: 700, margin: 0 }}>Email Sent</p>
                <p style={{ color: 'var(--text-dark)', margin: '0.2rem 0 0', opacity: 0.85 }}>{resendStatus}</p>
              </div>
            )}

            <form onSubmit={handleVerify} style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
              <div className="form-group" style={{ margin: 0 }}>
                <label style={{ fontWeight: 600, fontSize: '0.85rem', marginBottom: '0.4rem', display: 'block', color: 'var(--text-dark)' }}>Verification Code*</label>
                <input 
                  type="text" 
                  ...
                  maxLength={6}
                  style={{ padding: '0.85rem 1rem', fontSize: '1.2rem', textAlign: 'center', letterSpacing: '4px', background: '#fff', border: '1px solid var(--border-color)', borderRadius: '8px', width: '100%', outline: 'none', boxSizing: 'border-box', fontWeight: 700 }}
                  required 
                />
              </div>
            ...
            <div style={{ textAlign: 'center', marginTop: '1rem', fontSize: '0.85rem' }}>
              <button 
                type="button" 
                onClick={() => { setShowVerificationScreen(false); setLocalError(''); setResendStatus(''); }}
